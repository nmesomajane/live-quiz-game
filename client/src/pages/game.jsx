/**
 * GAME PAGE — Live Game Interface
 *
 * The main game screen. Chat-style layout with:
 *
 * DESKTOP LAYOUT:
 * ┌─────────────────────────────────────┬──────────────────────┐
 * │  MAIN (chat area)                   │  SIDEBAR             │
 * │  ┌──────────────────────────────┐   │  Timer               │
 * │  │ QuestionCard (sticky top)    │   │  ─────               │
 * │  ├──────────────────────────────┤   │  PlayerList          │
 * │  │ ChatWindow (scrollable feed) │   │  (scores + attempts) │
 * │  ├──────────────────────────────┤   │  ─────               │
 * │  │ InputBar / MasterLabel       │   │  [Master: Q form]    │
 * │  └──────────────────────────────┘   │                      │
 * └─────────────────────────────────────┴──────────────────────┘
 *
 * The WinnerBanner overlays the chat area when a round ends.
 *
 * STATUS-BASED RENDERING:
 *   "waiting" → game master sees QuestionForm in sidebar to start next round
 *   "active"  → timer running, players can guess
 *   "ended"   → WinnerBanner shown, round-reset coming in 5s
 */
...
export default function Game() {
  useSocket(); // keep all socket listeners active
  ...
  // Guard: no session means they landed here directly — send home
  useEffect(() => {
    ...
    <div className="h-screen bg-navy flex flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between px-5 py-3 border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Code badge */}
          <span className="text-xs font-display font-bold text-indigo tracking-widest
            bg-indigo/10 border border-indigo/20 px-3 py-1 rounded-lg">
            {session.code}
          </span>
      ...
      <div className="flex flex-1 min-h-0 gap-0">
        ...
        <div className="flex flex-col flex-1 min-h-0 border-r border-border relative">
          ...
          {/* Winner banner overlays chat (absolute so it doesn't shift layout) */}
        ...
        <div className="w-64 shrink-0 flex flex-col overflow-y-auto bg-card/30">
          ...
          {/* Master controls — shown in sidebar when waiting for next round */}
          {isMaster && isWaiting && (
            <div className="p-4">
              <QuestionForm />
            </div>
          ...
          {/* Non-master waiting message between rounds */}
          {!isMaster && isWaiting && (
            <div className="p-4 text-center">
              <p className="text-muted text-xs font-body">
                Waiting for{" "}
                <span className="text-indigo">{session.masterName}</span>
                <br />to set the next question…
              </p>
